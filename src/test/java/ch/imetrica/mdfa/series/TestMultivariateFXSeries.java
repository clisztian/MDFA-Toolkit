...
public class TestMultivariateFXSeries {
	...
	@Test
	public void testPrefiltering() throws Exception {
		...
		fxSeries.setWhiteNoisePrefilters(50);
		for(int i = 0; i < fxSeries.getNumberSeries(); i++) {
			assertTrue(fxSeries.getSeries(i).isPrefiltered());
			assertEquals(3, fxSeries.getSeries(i).getNumberPrefilterCoefficientSets());
		}
		...
		fxSeries.prefilterActivate(false);
		for(int i = 0; i < fxSeries.getNumberSeries(); i++) {
			
			assertFalse(fxSeries.getSeries(i).isPrefiltered());
			assertEquals(69, fxSeries.getSeries(i).getCoefficientSet(i).length);
		
		}
		

		fxSeries.prefilterActivate(true);
		fxSeries.computeAllFilterCoefficients();
				
		for(int i = 0; i < fxSeries.getNumberSeries(); i++) {
			
			assertEquals(50, fxSeries.getSeries(i).getPrefilterSet(i).length);
			assertEquals(3, fxSeries.getSeries(i).getNumberPrefilterCoefficientSets());
			assertTrue(fxSeries.getSeries(i).isPrefiltered());
			assertEquals(69, fxSeries.getSeries(i).getCoefficientSet(i).length);
		}
